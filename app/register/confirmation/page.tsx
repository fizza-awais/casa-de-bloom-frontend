...
function ConfirmationContent() {
  ...
  const recordType = (params.get("recordType") ??
    (participantType === "volunteer" ? "volunteer" : "registration")) as "registration" | "volunteer";
  const recordId =
    params.get("recordId") ??
  ...
  useEffect(() => {
    ...
    const loadRegistration = async () => {
      ...
      try {
        ...
        if (active) {
          setLoadError("We could not load the saved registration details.");
        }
      } finally {
        if (active) {
          setIsLoading(false);
  ...
  const resolvedEventDate = event?.event_date ?? "";
  const eventLabel = event?.name ?? "Event details";
  const formattedDate = formatEventDate(resolvedEventDate);
            ...
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-ui-text-muted mt-1">
                  Welcome to Casa de Bloom, <span className="font-semibold text-brand-dark">{displayName}</span>.
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 space-y-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="rounded-xl border border-ui-border bg-white/70 p-4 space-y-3">
                ...
                {isLoading ? (
                  <div className="flex items-center gap-3 rounded-xl border border-ui-border bg-white/70 p-4">
